...
class ScheduleCreatorQuickTimerCard extends HTMLElement {
  ...
  render() {
    ...
    if (!entity) {
      ...
      body = active.map((t) => `<div class="qt-active"><div class="qt-active-copy"><span class="qt-countdown" data-expiry="${qtEsc(t.expires_at)}">${qtDuration((new Date(t.expires_at) - Date.now()) / 1000)}</span><span class="qt-muted">${qtEsc(describeAction(t.action))} · poi torna allo stato precedente</span></div>${admin ? `<button type="button" class="qt-cancel" data-cancel="${qtEsc(t.id)}">Annulla</button>` : ''}</div>`).join('');
    } else if (!admin) {
