...
4. In **Impostazioni → Dashboard → ⋮ → Risorse**, aggiungi una risorsa di tipo
   **Modulo JavaScript** con URL
   `/schedule_creator/frontend/schedule-creator-card.js?v=0.3.1`. Se la risorsa
   esiste gia, modificala senza duplicarla. Se HACS ha aggiunto `?hacstag=1`,
   mantieni il parametro e aggiungi `&v=0.3.1` alla fine. Ricarica la pagina.
...
`/schedule_creator/frontend/schedule-creator-card.js?v=0.3.1`
...
version, for example `v0.3.1`. If the badge is absent or shows an earlier
